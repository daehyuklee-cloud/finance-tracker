...
// Smart modal — closes freely, confirms only if dirty
...
function EnvelopeView({ bank, bankId, setBanks, tags, allBanks }) {
  ...
  return (
    <div style={{marginTop:12}}>
      ...
      {showAdd&&<Modal title="New Envelope" onClose={()=>setShowAdd(false)} isDirty={!!envName||!!envBal||!!envGoal}><div style={{fontSize:13,color:T.subtext,marginBottom:12}}>Available: <strong style={{color}}>{sym(currency)}{fmtNum(unallocEnv?.balance||0)}</strong></div><Inp label="Name" value={envName} onChange={e=>setEnvName(e.target.value)} placeholder="e.g. Rent, Emergency"/><div style={{fontSize:12,color:T.subtext,marginBottom:4}}>Icon</div><EmojiPicker value={envEmoji} onPick={setEnvEmoji}/><Inp label="Allocate Amount" type="number" value={envBal} onChange={e=>setEnvBal(e.target.value)} placeholder="0.00"/><Inp label="Goal (optional)" type="number" value={envGoal} onChange={e=>setEnvGoal(e.target.value)} placeholder="e.g. 10000"/><Btn color={color} onClick={addEnvelope} style={{width:"100%"}}>Create Envelope</Btn></Modal>}
      {editEnv&&<Modal title="Edit Envelope" onClose={()=>setEditEnv(null)} isDirty={true}><Inp label="Name" value={editEnv.name} onChange={e=>setEditEnv(v=>({...v,name:e.target.value}))}/><div style={{fontSize:12,color:T.subtext,marginBottom:4}}>Icon</div><EmojiPicker value={editEnv.emoji} onPick={em=>setEditEnv(v=>({...v,emoji:em}))}/><Inp label="Goal (blank to remove)" type="number" value={editEnv.goal} onChange={e=>setEditEnv(v=>({...v,goal:e.target.value}))}/><Btn color={color} onClick={saveEnvEdit} style={{width:"100%"}}>Save</Btn></Modal>}
...
function CustomTotal({ banks, investments }) {
  ...
  const toggleBank=(bankId)=>{
    setSelected(s=>{
      const selecting = !s[`bank_${bankId}`];
      const next = {...s, [`bank_${bankId}`]: selecting};
      if(selecting){
        // deselect all envelopes of this bank
        banks.find(b=>String(b.id)===String(bankId))?.envelopes.forEach(e=>{ next[`env_${bankId}_${e.id}`]=false; });
      }
      return next;
    });
  };

  const toggleEnv=(bankId, envId)=>{
    setSelected(s=>{
      const selecting = !s[`env_${bankId}_${envId}`];
      const next = {...s, [`env_${bankId}_${envId}`]: selecting};
      if(selecting){
        // deselect parent bank
        next[`bank_${bankId}`]=false;
      }
      return next;
    });
  };
